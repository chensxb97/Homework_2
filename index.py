    ...
    for file in sorted_files:
        ...
        for token in tokens:
            if token in index_dict.keys():
                ...
                if int(file) not in index_dict[token]:
                    index_dict[token].append(int(file))
            else:
                index_dict[token] = [int(file)]

    # Sort dictionary
    temp_index_dict = sorted(index_dict.items())
    # Note temp_index_dict is are key-sorted tuples of the indexed dictionary
    sorted_index_dict = create_final_dict(temp_index_dict)
